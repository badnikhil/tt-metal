...
class LoopPlan:
    ...
    explicit: Optional[Tuple[Dict[str, int], ...]] = None
    ...
    def calls(self, bank: Mapping[str, int]) -> List[KernelInvocation]:
        if self.explicit is not None:
            return [KernelInvocation(**entry) for entry in self.explicit]
        return [
            KernelInvocation(
                **{
    ...
    def emit_calls(
        ...
        if self.explicit is not None:
            return "".join(
                render(KernelInvocation(**{k: str(v) for k, v in entry.items()}))
