...
class ComputePipeline:
    ...
    def _apply_loop_spec(plan, loop_spec):
        ...
        has_lists = any(isinstance(v, list) for v in slot_overrides.values())
        if has_lists:
            list_len = next(
                len(v) for v in slot_overrides.values() if isinstance(v, list)
            )
            explicit = []
            for i in range(list_len):
                entry = {}
                for slot, value in slot_overrides.items():
                    entry[slot] = value[i] if isinstance(value, list) else value.base
                explicit.append(entry)
            return replace(plan, call_levels=(), explicit=tuple(explicit))
        ...
        overrides = {
            slot: SlotIndex(
                base=spec.base,
                multipliers={
                    v: m for v, m in spec.multipliers.items() if v in declared
                },
            )
            for slot, spec in slot_overrides.items()
        }
        return replace(plan, slots={**plan.slots, **overrides})
