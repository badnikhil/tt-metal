...
def test_indexer_score_dsa_compressed_key_ratio4(device):
    """DeepSeek V4: query positions stay in token units while K/output are compressed rows.
    Exercises all four ratio-4 in-tile causal mask residues."""
    heads, dim, sq, t = 8, 128, 64, 64
    chunk_start = 96  # token units; two q tiles traverse residues 24 then 0 in compressed-key tiles
...
def test_indexer_score_csa_ratio4_block_cyclic(mesh_device):
    """V4 compressed keys retain GLM's block-cyclic layout while queries stay in token space."""
