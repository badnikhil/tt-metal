...
// (minimal_matmul_strided_reduce_scatter_async); see the note at the top of its header.
//
// Two deliberate differences from that helper:
//
//  1. The CCL-fusion branches (MinimalMatmulFusedOpSignaler / StridedReduceScatterFusedOpSignaler)
//     are not translated. The standalone op always passed empty signalers, so those paths were
//     unreachable from here.
//  2. Every buffer address is pushed as a Buffer* rather than ->address(), which registers a
//     BufferBinding, so the descriptor never carries a baked-in address.
//
// Keep the runtime-arg layout below in step with the kernels' get_arg_val order; the layout
// constants are shared with override_runtime_arguments and guarded at the end of
// create_descriptor.
