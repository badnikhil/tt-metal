...
/** Compressed K tile containing the per-row causal boundary for this 32-query-row tile.
 *  Query placement and straddle are in 32-token tiles; K is in compressed rows. The exact element threshold
 *  inside this tile is key_idx < floor((absolute_query_position + 1) / key_compression_ratio). */
