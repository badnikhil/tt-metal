...
struct operation_attributes_t {
    ...
    // Query tokens represented by one K-cache row. DSA supports 1 (legacy) and 4 (DeepSeek V4). Query
    ...
    // rows (the public block_cyclic_chunk_local argument remains query-token rows). HASHED (sp/chunk_local shape the
    // reader binary via compile-time arguments). nullopt == contiguous K (which is also what sp == 1 resolves to, since
