...
void bind_div(
    ...
    auto doc = fmt::format(
        ...
        Keyword args:
            ...
            dtype (ttnn.DataType, optional): dtype of the output tensor. Defaults to the dtype of the tensor operand. Integer division with `rounding_mode=None` only accepts `None` or `ttnn.float32`, and always yields `ttnn.float32`.
            output_tensor (ttnn.Tensor, optional): preallocated output tensor. Defaults to `None`.
